...
const NoteOnDesk: React.FC<NoteOnDeskProps> = ({ note, canDelete, onRead, onDelete }) => {
  const [hover, setHover] = useState(false);

  return (
    <div
      style={{ ...S.note(hover), left: `${note.x * 100}%`, top: `${note.y * 100}%` }}
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
      onClick={onRead}
      title="Klicken zum Lesen"
    >
      {canDelete && hover && (
        <button
...
const DeskModal: React.FC = () => {
  ...
  // Notizen laden, wenn Tisch geöffnet wird
  useEffect(() => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    // Notiz-Größe: 90px × ~80px — Mitte als Ankerpunkt nutzen
    const noteW = 90 / rect.width;
    const noteH = 80 / rect.height;
